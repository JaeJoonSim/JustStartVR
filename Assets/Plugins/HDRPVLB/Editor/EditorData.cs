    ...
    public class EditorData : ScriptableObject
    {
        ...
        public static EditorData Instance
        {
            get
            {
                if (ms_Instance == null)
                {
                    ms_Instance = Resources.Load<EditorData>("HDRPVLBEditorData");
                    Debug.Assert(ms_Instance != null, "Failed to find asset 'HDRPVLBEditorData', please reinstall the 'Volumetric Light Beam' plugin.");
                }
                return ms_Instance;
            }
        }
