...
/// ****************************************
struct HDRPVLB_appdata
{
    float4 vertex : POSITION;
    float4 texcoord : TEXCOORD0;

#if HDRPVLB_INSTANCING_API_AVAILABLE && (HDRPVLB_STEREO_INSTANCING || HDRPVLB_GPU_INSTANCING)
...
#if HDRPVLB_NOISE_3D || OPTIM_VS
    float4 uvwNoise_intensity : TEXCOORD5;
#endif

#if HDRPVLB_DEPTH_BLEND || HDRPVLB_DITHERING
    float4 projPos : TEXCOORD6;
#endif

#ifdef HDRPVLB_FOG_UNITY_BUILTIN_COORDS
    UNITY_FOG_COORDS(7)
#endif

#if HDRPVLB_INSTANCING_API_AVAILABLE
#if HDRPVLB_GPU_INSTANCING
    UNITY_VERTEX_INPUT_INSTANCE_ID // not sure this one is useful
#endif

#if HDRPVLB_STEREO_INSTANCING
    UNITY_VERTEX_OUTPUT_STEREO // for Single Pass Instanced rendering
#endif
#endif // HDRPVLB_INSTANCING_API_AVAILABLE
};
...
inline float ComputeBoostFactor(float pixDistFromSource, float outsideBeam, float isCap)
{
    ...
    float glareFrontal = HDRPVLB_GET_PROP(_GlareFrontal);
    float insideBoostDistance = glareFrontal * HDRPVLB_GET_PROP(_DistanceFallOff).y;
    ...
    float4 cameraParams = HDRPVLB_GET_PROP(_CameraParams);
...
float ComputeFresnel(float3 posObjectSpace, float3 vecCamToPixOSN, float outsideBeam, float boostFactor)
{
    ...
    float2 coneSlopeCosSin = HDRPVLB_GET_PROP(_ConeSlopeCosSin);
    ...
    float coneApexOffsetZ = HDRPVLB_GET_PROP(_ConeApexOffsetZ);
    ...
    float3 localForwardDirN = HDRPVLB_GET_PROP(_LocalForwardDirection);
...
inline float ComputeFadeWithCamera(float3 posViewSpace, float enabled)
{
    ...
    float camFadeDistEnd = camFadeDistStart + HDRPVLB_GET_PROP(_DistanceCamClipping);
...
half4 ComputeColor(float pixDistFromSource, float outsideBeam)
{
    ...
    float distanceFadeEnd = HDRPVLB_GET_PROP(_DistanceFallOff).y;
    float4x4 colorGradientMatrix = HDRPVLB_GET_PROP(_ColorGradientMatrix);
...
#else
    half4 color = HDRPVLB_GET_PROP(_ColorFlat);
#endif

    half alphaInside  = HDRPVLB_GET_PROP(_AlphaInside);
    half alphaOutside = HDRPVLB_GET_PROP(_AlphaOutside);
...
inline float3 ComputeVectorCamToPixOSN(float3 pixPosOS, float3 cameraPosOS)
{
    ...
    return lerp(vecCamToPixOSN, vecCamForwardOSN, HDRPVLB_CAMERA_ORTHO);
}

inline float GetTiltDistanceFactor(float3 posObjectSpace)
{
    float2 tiltVector = HDRPVLB_GET_PROP(_TiltVector);
...
v2f vertShared(HDRPVLB_appdata v, float outsideBeam)
{
    v2f o;
    ...
    #if HDRPVLB_STEREO_INSTANCING
      #ifndef HDRPVLB_SRP_API // TODO CHECK THAT WE DON'T NEED THIS WITH SRP
    ...
    float2 coneRadius = HDRPVLB_GET_PROP(_ConeRadius);
    ...
    float3 scaleObjectSpace = float3(maxRadius, maxRadius, HDRPVLB_GET_PROP(_DistanceFallOff).z); // maxGeometryDistance
    ...
    o.posWorldSpace = HDRPVLBObjectToWorldPos(vertexOS);
    o.posClipSpace = HDRPVLBObjectToClipPos(vertexOS.xyz);
    // TODO Should create and use HDRPVLBWorldToClipPos instead
    //o.posClipSpace = HDRPVLBWorldToClipPos(o.posWorldSpace.xyz);

#if defined(HDRPVLBWorldToViewPos)
    float3 posViewSpace = HDRPVLBWorldToViewPos(o.posWorldSpace.xyz);
#elif defined(HDRPVLBObjectToViewPos)
    float3 posViewSpace = HDRPVLBObjectToViewPos(vertexOS);
#else
    You_should_define_either_HDRPVLBWorldToViewPos_or_HDRPVLBObjectToViewPos
#endif
    ...
    float3 cameraPosObjectSpace = HDRPVLBGetCameraPositionObjectSpace(scaleObjectSpace);
    ...
    float3 distancesFallOff = HDRPVLB_GET_PROP(_DistanceFallOff);
    float attenuationLerpLinearQuad = HDRPVLB_GET_PROP(_AttenuationLerpLinearQuad);
...
half4 fragShared(v2f i, float outsideBeam)
{
...
#if HDRPVLB_INSTANCING_API_AVAILABLE && HDRPVLB_STEREO_INSTANCING
    ...
    {
        float4 addClippingPlaneWS = HDRPVLB_GET_PROP(_AdditionalClippingPlaneWS);
    ...
    {
        float4 clippingPlaneWS = HDRPVLB_GET_PROP(_DynamicOcclusionClippingPlaneWS);
        float distToClipPlane = DistanceToPlane(i.posWorldSpace.xyz, clippingPlaneWS.xyz, clippingPlaneWS.w);
        clip(distToClipPlane);
        float fadeDistance = HDRPVLB_GET_PROP(_DynamicOcclusionClippingPlaneProps);
...
#elif HDRPVLB_OCCLUSION_DEPTH_TEXTURE
    {
        ...
        float apexDist = HDRPVLB_GET_PROP(_ConeApexOffsetZ);
        float2 uv = normalize(i.posObjectSpace.xy) * 0.5 + 0.5;
        float dynamicOcclusionDepthRaw = tex2D(_DynamicOcclusionDepthTexture, uv).r;
        dynamicOcclusionDepthRaw = lerp(dynamicOcclusionDepthRaw, 1.0f - dynamicOcclusionDepthRaw, _HDRPVLB_UsesReversedZBuffer);

        float fallOffEnd = HDRPVLB_GET_PROP(_DistanceFallOff).z; // maxGeometryDistance

        float near = max(apexDist, kMinNearClipPlane);
        float far = near + fallOffEnd;

        float dynamicOcclusionDepthLinear = HDRPVLB_ZBufferToLinear(dynamicOcclusionDepthRaw, near, far);
        dynamicOcclusionDepthLinear -= apexDist;

        float fadeDistance = HDRPVLB_GET_PROP(_DynamicOcclusionDepthProps);
...
#endif // HDRPVLB_NOISE_3D

    // DEPTH BLEND
#if HDRPVLB_DEPTH_BLEND
    {
        float depthBlendDistance = HDRPVLB_GET_PROP(_DepthBlendDistance);
        
    #if FIX_DISABLE_DEPTH_BLEND_WITH_OBLIQUE_PROJ
        // disable depth sampling with oblique projection
        float3 nearPlaneWS = HDRPVLBFrustumPlanes[4].xyz;
        float3 farPlaneWS = HDRPVLBFrustumPlanes[5].xyz;
    ...
    float pixDistFromSourceTilted = GetTiltDistanceFactor(i.posObjectSpace);
    {
        ...
        // no intensity for cap if _DrawCap = 0
        intensity *= isEqualOrGreater(HDRPVLB_GET_PROP(_DrawCap), isCap - 0.00001);
        ...
        // attenuation
        float3 distancesFallOff = HDRPVLB_GET_PROP(_DistanceFallOff);
        intensity *= ComputeAttenuation(pixDistFromSourceTilted, distancesFallOff.x, distancesFallOff.y, HDRPVLB_GET_PROP(_AttenuationLerpLinearQuad));
    ...
    {
        float fadeWithCameraEnabled = 1 - HDRPVLB_CAMERA_ORTHO; // fading according to camera eye position doesn't make sense with ortho camera
    ...
    float2 ditherCoord = screenPos * _ScreenParams.xy * _HDRPVLB_DitheringNoiseTex_TexelSize.xy;
    float dither = tex2D(_HDRPVLB_DitheringNoiseTex, ditherCoord).r - 0.5;
    color += (1 - saturate(intensity)) * _HDRPVLB_DitheringFactor * dither;
#endif
...
#ifdef HDRPVLB_FOG_APPLY
    HDRPVLB_FOG_APPLY(color);
#endif
    return color;
}
