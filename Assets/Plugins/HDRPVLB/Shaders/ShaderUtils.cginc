...
#ifndef _HDRPVLB_SHADER_UTILS_INCLUDED_
#define _HDRPVLB_SHADER_UTILS_INCLUDED_

#include "ShaderMaths.cginc"

// https://docs.unity3d.com/Manual/SL-UnityShaderVariables.html
#define HDRPVLB_CAMERA_NEAR_PLANE _ProjectionParams.y
#define HDRPVLB_CAMERA_FAR_PLANE _ProjectionParams.z
#define HDRPVLB_CAMERA_ORTHO unity_OrthoParams.w // w is 1.0 when camera is orthographic, 0.0 when perspective

// Z buffer to linear depth
float HDRPVLB_ZBufferToLinear(float depth, float near, float far)
{
    float x = 1 - far / near;
    float y = far / near;
...
{
    float rawDepth = HDRPVLBSampleDepthTexture(uv);
    float linearDepthPersp = HDRPVLBLinearEyeDepth(rawDepth);

    rawDepth = lerp(rawDepth, 1.0f - rawDepth, _HDRPVLB_UsesReversedZBuffer);
    float linearDepthOrtho = (HDRPVLB_CAMERA_FAR_PLANE - HDRPVLB_CAMERA_NEAR_PLANE) * rawDepth + HDRPVLB_CAMERA_NEAR_PLANE;

    return lerp(linearDepthPersp, linearDepthOrtho, HDRPVLB_CAMERA_ORTHO);
}
...
{
    float sceneZ = max(0, SampleSceneZ_Eye(projPos) - HDRPVLB_CAMERA_NEAR_PLANE);
    float partZ = max(0, projPos.z - HDRPVLB_CAMERA_NEAR_PLANE);
    return saturate((sceneZ - partZ) / distance);
}
#endif // HDRPVLB_DEPTH_BLEND || HDRPVLB_DITHERING


#if HDRPVLB_NOISE_3D
uniform sampler3D _HDRPVLB_NoiseTex3D;
uniform float _HDRPVLB_NoiseCustomTime;

float3 Noise3D_GetUVW(float3 posWorldSpace, float3 posLocalSpace)
{
    float4 noiseVelocityAndScale = HDRPVLB_GET_PROP(_NoiseVelocityAndScale);
    float2 noiseParam = HDRPVLB_GET_PROP(_NoiseParam);
    ...
    // use _HDRPVLB_NoiseCustomTime if it's equal or higher than 0.0
    float currentTime = lerp(_Time.y, _HDRPVLB_NoiseCustomTime, isEqualOrGreater(_HDRPVLB_NoiseCustomTime, 0.0f));
...
float Noise3D_GetFactorFromUVW(float3 uvw)
{
	...
	float noise = tex3D(_HDRPVLB_NoiseTex3D, uvw).a;
...
#if HDRPVLB_COLOR_GRADIENT_MATRIX_HIGH
...
#elif HDRPVLB_COLOR_GRADIENT_ARRAY
...
#endif // HDRPVLB_COLOR_GRADIENT_*

#endif // _HDRPVLB_SHADER_UTILS_INCLUDED_
