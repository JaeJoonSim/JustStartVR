...
namespace HDRPVLB
{
    public class ShaderGenerator : ScriptableObject
    {
        ...
        const string kShaderAssetName = "HDRPVLBGeneratedShader";
        ...
        public class GenPass
        {
            ...
            public string Generate(SRPHelper.RenderPipeline rp, RenderingMode rm, EnabledFeatures enabledFeatures, int passID, int passCount)
            {
                ...
                code = code.Replace("{HDRPVLB_GEN_CULLING}", m_CullMode.ToString());
                code = code.Replace("{HDRPVLB_GEN_PRAGMA_INSTANCING}", rm == RenderingMode.GPUInstancing ? "#pragma multi_compile_instancing" : "");
                code = code.Replace("{HDRPVLB_GEN_PRAGMA_FOG}", IsFogSupported(rp) ? "#pragma multi_compile_fog" : "");
                ...
                code = code.Replace("{HDRPVLB_GEN_PRAGMA_MULTI_COMPILE_VARIANTS}", multiCompileVariants);

                var lang = GetShaderLangage(rp);
                code = code.Replace("{HDRPVLB_GEN_PROGRAM_PRE}", GetShaderLangagePre(lang));
                code = code.Replace("{HDRPVLB_GEN_PROGRAM_POST}", GetShaderLangagePost(lang));

                var passPre = "";

                if (passCount > 1)
                {
                    code = code.Replace("{HDRPVLB_GEN_INPUT_VS}", string.Format("{0}", passID));
                    code = code.Replace("{HDRPVLB_GEN_INPUT_FS}", string.Format("{0}", passID));
                }
                else
                {
                    code = code.Replace("{HDRPVLB_GEN_INPUT_VS}", string.Format("{0}", "v.texcoord.y"));
                    code = code.Replace("{HDRPVLB_GEN_INPUT_FS}", string.Format("{0}", "i.cameraPosObjectSpace_outsideBeam.w"));
                    passPre += "                #define HDRPVLB_PASS_OUTSIDEBEAM_FROM_VS_TO_FS 1" + System.Environment.NewLine;
                }

                if (rp != SRPHelper.RenderPipeline.BuiltIn)
                {
                    passPre += "                #define HDRPVLB_SRP_API 1" + System.Environment.NewLine;

                    if (rm == RenderingMode.SRPBatcher)
                    {
                        passPre += "                #define HDRPVLB_SRP_BATCHER 1" + System.Environment.NewLine;
                ...
                {
                    passPre += "                #define HDRPVLB_DITHERING 1" + System.Environment.NewLine;
                }

                passPre += LoadText(Instance.GetTextAssetIncludes(rp));

                code = code.Replace("{HDRPVLB_GEN_PRE}", passPre);
        ...
        class GenShader
        {
            ...
            {
                var shaderName = string.Format("Hidden/HDRPVLB_{0}_{1}", m_RenderPipeline, m_RenderingMode);
                var code = LoadText(Instance.textAssetBase);
                code = code.Replace("{HDRPVLB_GEN_SHADERNAME}", shaderName);
                ...
                code = code.Replace("{HDRPVLB_GEN_PASSES}", passes);
                code = code.Replace("{HDRPVLB_GEN_SPECIFIC_INCLUDE}", GetRenderPipelineInclude(m_RenderPipeline));
